...
local helpers        = require('tl_helpers');
...
local function drawArc(x1, y1, z1, x2, y2, z2, color, progress, orb)
    ...
    vertexBuffer:Unlock();
    arcTex = arcTex or getTexture(addon.path .. 'vendor/targetlines/assets/beam.png');
    ...
    if (orb) then
        orbTex = orbTex or getTexture(addon.path .. 'vendor/targetlines/assets/orb.png');
